...
public class DriveStraightConfigurator {
	...
	public void configDrivetrainDriveStraight() {
		...
		leftConfig.neutralDeadband = Constants.Drivetrain.kNeutralDeadband;
		rightConfig.neutralDeadband = Constants.Drivetrain.kNeutralDeadband;
	...
	public void setRobotDistanceConfigs(TalonFXInvertType leaderInvertType, TalonFXConfiguration leaderConfig) {
		...
		if (leaderInvertType == TalonFXInvertType.Clockwise) {
			 ...
			 * Diff: -1 *((+)Aux - (-)Master)| NOT OK, magnitude will be correct but
			 * negative
			 */

			leaderConfig.diff0Term = TalonFXFeedbackDevice.IntegratedSensor.toFeedbackDevice(); // Local Integrated
																								// Sensor
			leaderConfig.diff1Term = TalonFXFeedbackDevice.RemoteSensor0.toFeedbackDevice(); // Aux Selected Sensor
			leaderConfig.primaryPID.selectedFeedbackSensor = TalonFXFeedbackDevice.SensorDifference.toFeedbackDevice(); // Diff0
																														// -
																														// Diff1
		} else {
			...
			leaderConfig.primaryPID.selectedFeedbackSensor = TalonFXFeedbackDevice.SensorSum.toFeedbackDevice(); // Sum0
																													// +
																													// Sum1
		}
